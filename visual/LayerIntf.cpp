...
{
#if USE_NO_TRANSITION_ERROR
		//skip throw
		//if (name == "") {
		//	return;
		//}
TVPAddLog(ttstr(TJS_W("<<<<< tTJSNI_BaseLayer::StartTransition : ")) + name);
ttstr name_ = name;
if (name_ == TJS_W("rotatezoom") || 
name == TJS_W("rotatevanish") || 
name == TJS_W("ripple") || 
name == TJS_W("rotateswap") || 
name == TJS_W("universal")
) {
	//return;
	name_ = ttstr(TJS_W("crossfade"));
}
#endif
	// start transition
	...
	try
	{
		...
		pro = TVPFindTransHandlerProvider(name_);
